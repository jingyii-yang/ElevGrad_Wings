...
{
  dat$elevbin1 = ifelse(dat$MAX <= 2000, 1, 0) # the lowest MAX elev = 5m
  dat$elevbin2 = ifelse(dat$MAX >= 1000 & dat$MAX <= 3000, 1, 0)
  dat$elevbin3 = ifelse(dat$MAX >= 2000 & dat$MAX <= 4000, 1, 0)
  dat$elevbin4 = ifelse(dat$MAX >= 3000 & dat$MAX <= 5000, 1, 0)
  dat$elevbin5 = ifelse(dat$MAX >= 4000 & dat$MAX <= 6000, 1, 0)
  dat$elevbin6 = ifelse(dat$MAX >= 5000 & dat$MAX <= 7000, 1, 0)
#  dat$elevbin7 = ifelse(dat$MAX >= 6000, 1, 0) this band contains too few species (18) to be modelled.
}
...
elev_bin_100coef <- function(input, tag){
  ...
  for (ii in 1:length(n_trees)){
    ...
    saveRDS(coef_ntree, paste0('/rdsgpfs/general/user/jy419/home/_Wings_/results/SI4_elev_band_coef100_BW.2km/', tag, '.rds'))
  return(coef_ntree)
}
...
elev_bin_100coef(input = dat, tag = 'full (bw=2km)')
